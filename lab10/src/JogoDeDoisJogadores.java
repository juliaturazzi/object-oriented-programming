...
public abstract class JogoDeDoisJogadores {
    ...
    public int jogar() {
        ...
        Partida partidaTerminada = new Partida(new Date(),
                contVitoriasJogador1, contVitoriasJogador2,
    ...
    public String obterResultadoUltimoJogo() {
        if (this.historicoResultados.isEmpty())  return null;
        ...
        if (contRodadasVencidasJogador1 == contRodadasVencidasJogador2)
            ...
            return String.format("O jogador %s venceu o jogo por %d a %d", nomeVencedor, pontosVencedor, pontosPerdedor);
        }
    }

    public double getPercentualVitoriasJogador1() {
        double scoreJogador1 = 0;

        for (Partida resultado:historicoResultados)
            if (resultado.contRodadasVencidasJogador1>resultado.contRodadasVencidasJogador2)  scoreJogador1++;

        return (scoreJogador1/historicoResultados.size())*100;
    }

    public double getPercentualVitoriasJogador2() {
        double scoreJogador2 = 0;

        for (Partida resultado:historicoResultados)
            if (resultado.contRodadasVencidasJogador2>resultado.contRodadasVencidasJogador1)  scoreJogador2++;

        return (scoreJogador2/historicoResultados.size()*100);
    }
    ...
    public double getPercentualEmpates() {
        ...
        for (Partida resultado:historicoResultados)
            if (resultado.contRodadasVencidasJogador1==resultado.contRodadasVencidasJogador2)  empate++;

        return (empate/historicoResultados.size())*100;
    }
    ...
    private class Partida {
        ...
        Partida(Date data,
                int contRodadasVencidasJogador1, int contRodadasVencidasJogador2,
                int contEmpates) {
            this.data = data;
        ...
        int getResultado() {
            if (contRodadasVencidasJogador1 > contRodadasVencidasJogador2)  return VITORIA_JOGADOR_1;

            else if (contRodadasVencidasJogador1 < contRodadasVencidasJogador2)  return VITORIA_JOGADOR_2;
